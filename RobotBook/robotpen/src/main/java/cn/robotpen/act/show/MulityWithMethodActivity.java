...
import com.zhiyuweilai.tiger.robotpen.databinding.ActivityMulityWithmethodBinding;
...
public class MulityWithMethodActivity extends Activity
    ...
    ActivityMulityWithmethodBinding mulityWithmethodBinding;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        mulityWithmethodBinding = DataBindingUtil.setContentView(this, R.layout.activity_mulity_withmethod);
    ...
    public void onStart() {
        ...
        mulityWithmethodBinding.recordBoardView.beginPage();// xml创建的画布必须刷新一次
    }
    ...
    protected void onDestroy() {
        super.onDestroy();
        if (mulityWithmethodBinding.recordBoardView != null)
            mulityWithmethodBinding.recordBoardView.dispose(); // 退出Activity时将服务释放，方便其他地方继续使用
    }
    ...
    public void onClickcleanLineBut(View v) {
        mulityWithmethodBinding.recordBoardView.cleanTrail();
    }

    public void onClickcleanPhotoBut(View v) {
        mulityWithmethodBinding.recordBoardView.cleanPhoto();
    }

    public void onClickcleanScreenBut(View v) {
        mulityWithmethodBinding.recordBoardView.cleanScreen();
    }

    public void onClickinnerPhotoBut(View v) {

        mulityWithmethodBinding.recordBoardView.setDataSaveDir(ResUtils.getSavePath(ResUtils.DIR_NAME_DATA));
    ...
    public void onClickphotoScaleTypeBut(View v) {
        mulityWithmethodBinding.recordBoardView.setPhotoScaleType(ImageView.ScaleType.CENTER_CROP);
    }

    public void onClickremovePhotoBut(View v) {

        mulityWithmethodBinding.recordBoardView.delCurrEditPhoto();
    }

    public void onClickinnerbgBut(View v) {
        mulityWithmethodBinding.recordBoardView.setDataSaveDir(ResUtils.getSavePath(ResUtils.DIR_NAME_DATA));
        mulityWithmethodBinding.recordBoardView.startPhotoEdit(true); // 设置图片可以编辑状态
        Intent intent2 = new Intent(Intent.ACTION_PICK, android.provider.MediaStore.Images.Media.EXTERNAL_CONTENT_URI);
        startActivityForResult(Intent.createChooser(intent2, "选择背景"), SELECT_BG);
    }

    public void onClickbgScaleTypeBut(View v) {
        mulityWithmethodBinding.recordBoardView.setBgScaleType(ImageView.ScaleType.CENTER_CROP);
    }

    public void onClickremoveBgBut(View v) {
        mBgUri = null;
        mulityWithmethodBinding.recordBoardView.setBgPhoto(null);
    }

    public void onClicksaveScreenBut(View v) {
        mulityWithmethodBinding.recordBoardView.setSaveSnapshotDir(ResUtils.getSavePath(ResUtils.DIR_NAME_PHOTO));// 设置存储路径
        mulityWithmethodBinding.recordBoardView.saveSnapshot();
    }

    public void onClickrecordBut(View v) {
        mulityWithmethodBinding.recordBoardView.setSaveVideoDir(ResUtils.getSavePath(ResUtils.DIR_NAME_VIDEO));// 设置存储路径
        if (butFlag == 0) { // 点击开始录制按钮
            butFlag = 1;// 可以暂停
            ((Button)v).setText("暂停");
            mulityWithmethodBinding.recordStopBut.setClickable(true);
            mulityWithmethodBinding.recordStopBut.setBackgroundColor(Color.DKGRAY);
            mulityWithmethodBinding.recordBoardView.startRecord();
        } else if (butFlag == 1) {// 点击暂停按钮
            butFlag = 2;// 可以继续
            ((Button)v).setText("继续");
            mulityWithmethodBinding.recordBoardView.setIsPause(true);
        } else if (butFlag == 2) {// 点击继续按钮
            butFlag = 1;// 可以暂停
            ((Button)v).setText("暂停");
            mulityWithmethodBinding.recordBoardView.setIsPause(false);
        }
    }

    public void onClickrecordStopBut(View v) {
        butFlag = 0;// 可以暂停
        mulityWithmethodBinding.recordBut.setText("录制");
        v.setBackgroundColor(Color.GRAY);
        ((Button)v).setClickable(false);
        mulityWithmethodBinding.recordBoardView.endRecord();
    }

    public void onClickdelPageBut(View v) {
        mulityWithmethodBinding.recordBoardView.delCurrPage();
    }

    public void onClickgotoProBut(View v) {
        mulityWithmethodBinding.recordBoardView.frontPage();
    }

    public void onClickgotoNextBut(View v) {
        mulityWithmethodBinding.recordBoardView.nextPage();
    ...
    protected void initView() {
        mulityWithmethodBinding.recordBoardView.setDaoSession(RobotPenApplication.getInstance().getDaoSession());
        mulityWithmethodBinding.recordBoardView.setIsTouchWrite(true);
        // 获取关键服务
        mPenManage = mulityWithmethodBinding.recordBoardView.getPenManage();
        // 设置状态监听
    ...
    public void checkIntentInsertPhoto() {
        ...
        if (null != mInsertPhotoUri) {
            mulityWithmethodBinding.recordBoardView.insertPhoto(mInsertPhotoUri);
            mInsertPhotoUri = null;
        }
        if (null != mBgUri) {
            mulityWithmethodBinding.recordBoardView.setBgPhoto(mBgUri);
            mBgUri = null;
        }
    }
    ...
    private PenService.OnConnectStateListener onConnectStateListener = new PenService.OnConnectStateListener() {
        ...
        public void stateChange(String arg0, ConnectState arg1) {
            if (arg1 == ConnectState.CONNECTED) {
                ...
                // 刷新当前临时笔记
                mulityWithmethodBinding.recordBoardView.refresh();
    ...
    public void onEvent(WhiteBoardView.BoardEvent boardEvent) {
        switch (boardEvent) {
            ...
            case ERROR_DEVICE_TYPE:
                // 检查当前是否是临时笔记
                if (mulityWithmethodBinding.recordBoardView.getNoteEntity() != null
                        && NoteEntity.KEY_NOTEKEY_TMP.equals(mulityWithmethodBinding.recordBoardView.getNoteEntity().getNoteKey())) {
                    // 如果是，那么将临时笔记另存，并标识设备类型
                    mNoteKey = DeviceType.toDeviceType(mulityWithmethodBinding.recordBoardView.getNoteEntity().getDeviceType()).getDeviceIdent()
