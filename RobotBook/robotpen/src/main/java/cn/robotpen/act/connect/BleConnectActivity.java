...
import com.zhiyuweilai.tiger.robotpen.databinding.ActivityBleConnectBinding;
...
public class BleConnectActivity extends Activity {
    ...
    private ActivityBleConnectBinding bleConnectBinding;
    // int stateFlag = 0;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        bleConnectBinding = DataBindingUtil.setContentView(this, R.layout.activity_ble_connect);
        // mPenManage = new PenManage(this, SmartPenService.TAG);
        mHandler = new Handler();
        mHttpClient = new AsyncHttpClient();
        mPenAdapter = new PenAdapter(BleConnectActivity.this);
        bleConnectBinding.listview.setAdapter(mPenAdapter);
        bleConnectBinding.listview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
    ...
    public void onClickDisConnect(View view) {
        mPenManage.disconnectDevice();
        bleConnectBinding.scanBut.setVisibility(View.VISIBLE);
        bleConnectBinding.disconnectBut.setVisibility(View.GONE);
        bleConnectBinding.statusText.setText("未连接设备!");
    }

    public void onClickDeviceDetail(View view) {
        Toast.makeText(BleConnectActivity.this, "持续完善中……", Toast.LENGTH_SHORT).show();
        bleConnectBinding.deviceDetail.setClickable(false);
    }
    ...
    private PenService.OnScanDeviceListener onScanDeviceListener = new PenService.OnScanDeviceListener() {
        ...
        public void complete(HashMap<String, DeviceEntity> list) {
            if (!mPenManage.getIsStartConnect()) {
                ...
                bleConnectBinding.scanBut.setEnabled(true);
            }
        }
        ...
        public void version(int hw, int sw) {
            mHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    if (device != null) {
                        bleConnectBinding.statusText.setText("已连接设备: " + device.getName());
                        bleConnectBinding.scanBut.setVisibility(View.GONE);
                        bleConnectBinding.disconnectBut.setVisibility(View.VISIBLE);
        ...
        public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
            ...
            if (StringUtil.isNotEmpty(ver)) {
                ...
                if (newVer >= mFirmwareVer) {
                    bleConnectBinding.deviceDetail.setText("固件版本：" + ver + "，可升级！");
                    bleConnectBinding.deviceDetail.setVisibility(View.VISIBLE);
                } else {
                    bleConnectBinding.deviceDetail.setVisibility(View.GONE);
                }
            }
        }
